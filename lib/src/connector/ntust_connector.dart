...
class NTUSTConnector {
  ...
  //static const String ntustLoginUrl = "https://stuinfosys.ntust.edu.tw/NTUSTSSOServ/SSO/Login/CourseSelection";
  ...
  static Future<Map<String, dynamic>> login(String account, String password) async {
    bool loadStop = false;
    try {
      bool checkLoginStatus(String page) {
        return page.contains("Issued") &&
            page.contains("Expires") &&
            page.contains("You are signed in as");
      }

      final parameter = ConnectorParameter(ntustLoginUrl);
      final ntustLoginPage = await Connector.getDataByGet(parameter);
      if(checkLoginStatus(ntustLoginPage)) {
        return {"status": NTUSTLoginStatus.success};
      }

      Document node = parse(ntustLoginPage);
      final verificationToken = node.querySelector('input[name="__RequestVerificationToken"]')?.attributes["value"];
      parameter.data = {
        "__RequestVerificationToken": verificationToken,
        "Username": account,
        "Password": password,
      };
      final resultPage = await Connector.getDataByPost(parameter);
      if(checkLoginStatus(resultPage)) {
        return {"status": NTUSTLoginStatus.success};
      }
    } catch (e, stack) {
  ...
  static void _oldLoginMethod() {
    ...
    //           source: 'document.getElementsByName("UserName")[0].value = "$account";');
    //       await webView?.evaluateJavascript(
    //           source: 'document.getElementsByName("Username")[0].value = "$account";');
    //       await webView?.evaluateJavascript(
    //           source: 'document.getElementsByName("Password")[0].value = "$password";');
    //       await webView?.evaluateJavascript(
    //           source: 'document.getElementById("btnLogIn").click(); document.getElementById("loginButton").click();');
