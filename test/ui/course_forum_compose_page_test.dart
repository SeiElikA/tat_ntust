...
void main() {
  ...
  testWidgets('內文是空的或只有空白時，送出是停用的', (tester) async {
    ...
    expect(tester.widget<ButtonStyleButton>(sendButton()).onPressed, isNull);
    ...
    await tester.pump();
    expect(tester.widget<ButtonStyleButton>(sendButton()).onPressed, isNotNull);
    expect(calls, 0);
  });
  ...
  testWidgets('頁尾說明只能發純文字，並留一個網頁入口', (tester) async {
    ...
    await tester.tap(buttonWithText(R.current.forumOpenInWeb));
    await tester.pumpAndSettle();
