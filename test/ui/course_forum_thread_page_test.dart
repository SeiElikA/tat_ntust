...
void main() {
  ...
  Finder replyButton() => find.widgetWithText(TextButton, R.current.forumReply);
  ...
  testWidgets('沒有任何一篇能回覆時：畫「不開放回覆」加網頁入口，不是一顆會失敗的鈕', (tester) async {
    ...
    expect(find.text(R.current.forumThreadLocked), findsOneWidget);
    final webButton = find.widgetWithText(TextButton, R.current.forumOpenInWeb);
  ...
  testWidgets('送出成功：新貼文併進討論串、吐「已送出」，重抓也成功', (tester) async {
    ...
    await tester.pump();
    await tester.tap(find.widgetWithText(TextButton, R.current.forumSend));
    await tester.pumpAndSettle();
